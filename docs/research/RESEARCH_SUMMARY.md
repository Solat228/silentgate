...
* **[REAL_APP_BYPASS.md](REAL_APP_BYPASS.md)** (17.08.2026) — обход туннеля по имени программы на
  Windows без драйвера. Redirect-слои WFP — только драйверу; единственный юзермод-кандидат
  `FwpmConnectionPolicyAdd0` жив, но не проверен против TUN-маршрута.
* **[NATIVE_DIRECT_MODE.md](NATIVE_DIRECT_MODE.md)** (19.09.2026) — «настоящий прямой режим» без
  TUN и без прокси, который приложения игнорируют. **Только драйвер ядра (Windows) или root
  (Android)** — все проксификаторы Windows без исключения возят `.sys`; «без прокси» для
  VLESS/Reality невозможно в принципе; macOS — единственная ОС, где это штатный API
  (`NETransparentProxyProvider`); Linux — штатно через cgroup/TPROXY/eBPF; iOS — тупик.
