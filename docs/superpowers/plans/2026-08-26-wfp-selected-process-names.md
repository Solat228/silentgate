...
- [x] **Шаг 1: Добавить падающие тесты на поля и материализацию.**
...
- [x] **Шаг 2: Запустить новые тесты и убедиться, что они падают.**
...
- [x] **Шаг 3: Реализовать поля и материализацию.**
...
- [x] **Шаг 4: Обновить тесты копирования плана.**
...
- [x] **Шаг 5: Запустить тесты задачи.**
...
- [x] **Шаг 6: Зафиксировать самостоятельный результат.**
...
- [x] **Шаг 1: Написать падающий round-trip-тест.**
...
- [x] **Шаг 2: Запустить тест и проверить отказ до реализации.**
...
- [x] **Шаг 3: Реализовать запись и чтение.**
...
- [x] **Шаг 4: Запустить тесты файла плана.**
...
- [x] **Шаг 5: Зафиксировать результат.**
...
- [x] **Шаг 1: Написать тесты классификации.**
...
- [x] **Шаг 2: Запустить тест до реализации.**
...
- [x] **Шаг 3: Реализовать чистый классификатор.**
...
- [x] **Шаг 4: Подключить классификатор к `_writeKillSwitchPlan`.**
...
- [x] **Шаг 5: Добавить исходный страж интеграции.**
...
- [x] **Шаг 6: Запустить тесты задачи.**
...
- [x] **Шаг 7: Зафиксировать результат.**
...
- [x] **Шаг 1: Зафиксировать ожидаемый жизненный цикл тестами/стражами.**
...
- [x] **Шаг 2: Ввести единый путь чтения и материализации плана.**
...
- [x] **Шаг 3: Обновить стартовый hold и разрешение LUID.**
...
- [x] **Шаг 4: Добавить периодический мониторинг выбранных имён в существующий тик.**
...
- [x] **Шаг 5: Не менять поведение смерти ядра и остановки.**
...
- [x] **Шаг 6: Запустить статические тесты помощника.**
...
- [x] **Шаг 1: Проверить, что обычная маршрутизация не изменилась.**
...
- [x] **Шаг 3: Запустить полный набор тестов.**
...
- [x] **Шаг 4: Проверить итоговый diff и отсутствие секретов.**
...
- [x] **Шаг 5: Сделать один итоговый коммит после зелёных проверок, если остались только мелкие тестовые правки.**
  ...
  *Итог: отдельный коммит не понадобился — дерево было чистым.*
...
- [x] **Шаг 1: Подготовить два тестовых процесса с одинаковым basename.**
  ...
  *Итог: в госте подняты две копии одного имени из разных каталогов; правило одно, по имени, реальные пути в правила не вносились.*

- [x] **Шаг 2: Проверить материализацию обоих путей.**
  ...
  *Итог: оба пути материализованы уже на базовом подъёме — с одним живым процессом 38 правил, с двумя 39; в плане `blockedAppNames` содержит только имя, `blockedAppPaths` пуст.*

- [x] **Шаг 3: Проверить запуск нового экземпляра после подключения.**
  ...
  *Итог: второй экземпляр, запущенный после подключения, добавлен одной заменой (39 → 40 правил, 86 → 90 фильтров), строка «состав обновлён под выбранные имена, правил на программы — 2»; снятия набора перед заменой в журнале нет.*

- [x] **Шаг 4: Проверить точное правило по пути.**
  ...
  *Итог: при `byName=false` в план ушёл только выбранный полный путь, второй экземпляр того же имени не материализован (39 правил против 40), наблюдение за именами не включалось вовсе.*

- [x] **Шаг 5: Проверить отказ обновления без потери старой защиты.**
  ...
  *Итог: отказ наведён подстановкой в план пути, который WFP не принимает; журнал: «отвергнуто …, код 5 — откатываю, остаётся прежний набор» и «обновить состав не удалось, прежняя защита ДЕРЖИТСЯ». Число фильтров не изменилось, цикл продолжил работу, ядро не остановлено; повторная одинаковая причина в журнал не попала.*

- [x] **Шаг 6: Проверить остановку ядра и явное отключение.**
  ...
  *Итог: принудительная остановка ядра оставила блокировку стоять (98 фильтров на месте, «ядро умерло, блокировка ДЕРЖИТСЯ»); по явному отключению — «блокировка снята», число фильтров вернулось к исходному.*

- [x] **Шаг 7: Погасить стенд и сохранить только безопасные результаты.**
  ...
  *Итог: временные задачи Планировщика удалены, VM остановлена; полные пути, адреса и токены в отчёт не выносились.*
