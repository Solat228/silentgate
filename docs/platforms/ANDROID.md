...
## ⚠️ ЭТО ПЛАН, А НЕ ДОСКА СОСТОЯНИЯ. Читать вместе с таблицей ниже

**Android выпущен с 1.5.0 и патчится по 1.9.3.** А чекбоксы в этом документе так и остались в
положении «не сделано»: 92 из 94. Их не поддерживали ни одного дня после начала работ — и это не
мелочь, потому что документ выглядит как отчёт о невыполненной работе там, где платформа уже год
как в руках у людей.

Чекбоксы ниже **сознательно оставлены как есть**: они фиксируют ЗАМЫСЕЛ на момент 27.07.2026, и
переставлять их задним числом значило бы потерять то единственное, ради чего этот файл ценен —
чем сделанное отличается от задуманного. Настоящее состояние — в таблице.

**Метод сверки (21.08.2026):** каждая задача проверялась по коду отдельным агентом, всё
помеченное «сделано» перепроверял второй агент, обязанный ОПРОВЕРГНУТЬ пометку. Доказательство —
только `файл:строка` с живым кодом. ⚠️ Комментарий, утверждающий, что нечто сделано,
доказательством не считался: в этом проекте устаревшее описание уже дважды прикрывало
отсутствующую реализацию (`CLAUDE.md`, 1.4.2 и 1.4.3).

**Итог: 97 задач.** Сделано — 35. Сделано иначе — 50. Не сделано — 11. Не установлено — 1.

⚠️ **«Сделано иначе» — самая большая графа, и это нормально, а не тревожно.** Половина плана
писалась до кода: где решение оказалось лучше задуманного (адаптивная вёрстка вместо отдельного
дерева `ui/mobile/`, один общий рецепт сборки ядер вместо двух скриптов), там расхождение и
записано. Тревожна другая графа — ниже.

### Что НЕ сделано — одиннадцать пунктов

| № | Чего нет | Доказательство |
|---|---|---|
| 6 | ⚠️ **не установлено:** матч Response Rule по UA `SilentGate/… (Android)` на боевой панели — по репозиторию не проверить | — |
| 7 | Документы не догнали код. ✅ `ROADMAP` и `ARCHITECTURE` поправлены 21.08.2026; **`REMNAWAVE_INTEGRATION.md` всё ещё советует отдавать нам base64** | `docs/REMNAWAVE_INTEGRATION.md:39` |
| 19а | ⚠️ **`VpnStatus.message` — свободная РУССКАЯ строка**, и главный экран печатает её как есть. В девяти языках из десяти человек видит русский текст ровно в момент обрыва | `engine/engine_base.dart:1538`, `ui/home_screen.dart:962` |
| 22 | 16 КБ page size (требование Play для targetSdk 35): ни `zipalign -P 16`, ни проверки выравнивания ELF | манифеста проверки нет нигде |
| 30а | Отказ от `POST_NOTIFICATIONS` не обработан: баннера «уведомления выключены» нет. ⚠️ С 20.08.2026 это дороже, чем было, — уведомление об обрыве появилось, и при отказе оно молча не покажется | `MainActivity.kt:460-467` |
| 42 | `PortCheck` на Android не вызывается — конфликт портов с Happ/v2rayNG не диагностируется | `AndroidEngine.startSession` его не зовёт |
| 59 | Нет `WAKE_LOCK`: длинные операции не удерживаются | `AndroidManifest.xml:11` |
| 59а | Обновление подписки — наивный `Timer.periodic` от старта процесса, не привязанный к сервису | `core/subscription/subscription_updater.dart:29` |
| 60а | Сканера QR нет вовсе (ни пакета камеры, ни разрешения `CAMERA`) | `ui/import_screen.dart:99-125` |
| 60б | Единого сценария первого запуска нет; `POST_NOTIFICATIONS` спрашивается молча на старте — **явно наоборот к плану** | `MainActivity.kt:460-467` |
| 68 | Кнопка «Обновить» у notice-серверов ищет РУССКИЕ слова (`нажмите`/`обнов`) — при англоязычном тексте панели не появится | `ui/widgets/server_tile.dart:277-281` |
| 75 | Флейворов сборки нет: `productFlavors` не встречается ни разу | `app/android/app/build.gradle.kts:20` |
| 79 | `docs/TEST_PLAN.md` озаглавлен «(Windows)» и слов `android`/`adb` не содержит вовсе | `docs/TEST_PLAN.md:1` |
| 81 | Обязательный чек-лист приёмки целиком не прогонялся; hysteria2 живьём не проверен ни на одной платформе | `ANDROID.md:470` |

⚠️ **Общий почерк у 19а и 68 один:** русский язык, зашитый в логику, а не в переводы. Первое —
вывод наружу, второе — разбор входа. Оба не ловятся ни компилятором, ни тестами локализации:
ключей ARB там нет и быть не может, потому что строка не проходит через ARB вовсе.

**Задачи фаз 2 и 3** (ядра, `VpnService`, `OpenTun`, `protect`, `FindConnectionOwner`, `onRevoke`,
генерационные гварды) проверены отдельно и закрыты практически полностью — из 25 задач не сделаны
только две: 22 и 42, обе в таблице выше.
