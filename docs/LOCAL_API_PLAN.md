...
### Задача 3b: маршрутизатор выходов для режима «Только прокси»

**Почему эта задача появилась.** Дефект дизайна, вскрытый на задаче 3: порты серверов живут в
конфиге sing-box, а sing-box на Windows стартует ТОЛЬКО при `captureMode == CaptureMode.tun`
(`windows_engine.dart`, ветка выбора захвата), и `tun`-инбаунд в его конфиге безусловен. Значит в
режиме «Только прокси» портов серверов нет физически — второе ядро там не запускается вовсе.
Спека обещала ровно их сочетание: машина мимо туннеля, а вызывающий выбирает страну на запрос.

**Решение владельца (11.08.2026):** в режиме «Только прокси» поднимать ОТДЕЛЬНЫЙ маленький
экземпляр sing-box — чистый маршрутизатор выходов, без `tun`-инбаунда. Механизм выходов тот же,
что уже проверен живьём; ничего второго не изобретается.

**Файлы:**
- Создать: `app/lib/core/singbox/exit_router_config_builder.dart`
- Изменить: `app/lib/engine/windows/windows_engine.dart`
- Тест: `app/test/exit_router_config_test.dart` (создать)

**Интерфейсы:**
- Потребляет: `ApiPorts`, `apiExitInboundTag` (задача 3); `ExitOutbounds.build`, `exitTagFor`
  (существующие); `AppSettings.apiExitServerKeys|apiToken|captureMode` (задачи 1-2).
- Производит: `ExitRouterConfigBuilder` с конструктором
  `ExitRouterConfigBuilder({required List<String> serverKeys, required String token, required List<Map<String, dynamic>> exitOutbounds})`
  и методами `Map<String, dynamic> buildMap()` / `String buildJson()`.

**Что строит конфиг.** Он крошечный и намеренно НЕ похож на TUN-конфиг:

- `inbounds` — по одному `mixed` на живой сервер, `listen: 127.0.0.1`, порт из `ApiPorts`,
  `users: [{username: 'sg', password: <токен>}]`;
- `outbounds` — переданные `exitOutbounds` (теги `exit-<fnv32>`) плюс `direct`;
- `route.rules` — на каждый инбаунд правило `inbound: [api-exit-…] → outbound: exit-…`;
- `route.final` — `direct`.

⚠️ **Ни `tun`-инбаунда, ни `dns`-секции, ни правил раздельного туннелирования здесь НЕТ, и это
не упущение.** В режиме «Только прокси» ничего не перехватывается: трафик приходит лишь в те
порты, куда вызывающий обратился сам. Правила «этот сайт — прямо» к явно выбранному порту не
применяются по тому же соображению, что и в задаче 3.

**Правила раздельного туннелирования — по отдельной галочке.** Решение владельца (11.08.2026):
в режиме «Только прокси» правила действуют ТОЛЬКО когда включена новая настройка
`AppSettings.applyRulesInProxyOnly` (bool, **умолчание `false`**).

Обоснование умолчания: в этом режиме раздельное туннелирование не действует ни для одной
программы машины — ничего не перехватывается. Применять правила к одному лишь API-порту по
умолчанию значило бы включить их там, где они больше нигде не работают, и удивить того, кто
явно попросил конкретный сервер. Кому нужна защита от собственного скрипта — включает галочку.

Что делает галочка, когда включена: в конфиг маршрутизатора добавляются блок-правила,
ограниченные инбаундами API (тем же способом, что `_addApiExitBlockGuard` в задаче 3), и они
стоят ВЫШЕ правил маршрутизации портов. Остальные действия («Прямо», «Туннель через другой
сервер») не добавляются: порт уже задаёт адресата явно, и переопределять его чужим правилом —
ровно то, от чего защищались в задаче 3.

⚠️ Новую настройку провести через все шесть точек `AppSettings` и назвать в `reconnectReasons`
(она запекается в конфиг при подъёме). Строку интерфейса — в ARB во все 10 языков; сам тумблер
показывать только при `captureMode == CaptureMode.proxyOnly`, иначе он видим и ничего не делает.

- [ ] **Шаг 1: Написать падающий тест** — состав конфига: инбаунд и правило на каждый живой
  сервер; отсутствие `tun`-инбаунда; отсутствие инбаунда при пустом токене; отсутствие инбаунда
  для сервера без живого outbound-а; `route.final == 'direct'`; валидный JSON.
- [ ] **Шаг 2: Убедиться, что тест падает** (`flutter test test/exit_router_config_test.dart`).
- [ ] **Шаг 3: Написать построитель.**
- [ ] **Шаг 4: Убедиться, что тесты проходят.**
- [ ] **Шаг 5: Поднять экземпляр в движке.** В `startSession`, в ветке `proxyOnly`: если
  `apiEnabled`, токен непуст и есть хоть один живой выход — записать конфиг и запустить
  `SingboxProcess` (тот же класс, что для hysteria2, БЕЗ прав администратора: туннеля нет).
  Гасить его в `teardownCore`/`cleanup` вместе с остальными ядрами.
  ⚠️ Процесс обязан гаситься и при сбое подъёма: осиротевший sing-box держит порты, и следующее
  подключение упрётся в конфликт.
- [ ] **Шаг 6: Проверить конфиг настоящим ядром** — `sing-box check -c` на выданном файле,
  ожидается код 0. ⚠️ Код 0 не доказывает существования тегов: целостность стережёт тест.
- [ ] **Шаг 7: Прогнать `flutter analyze` и `flutter test`.**
- [ ] **Шаг 8: Коммит.**
...
```json
  ...
  "captureProxyOnly": "Только прокси",
  "captureProxyOnlySub": "Ядро поднято, локальные порты слушают, но компьютер не в туннеле: через VPN идёт только тот, кто явно указал наш прокси",
  "apiRulesInProxyOnly": "Применять правила раздельного туннелирования",
  "apiRulesInProxyOnlySub": "В этом режиме правила по умолчанию не действуют ни для одной программы. Включите, если хотите, чтобы список «Блок» распространялся и на запросы через локальные порты."
```
...
- [ ] **Шаг 3b: ⚠️ ДОБАВИТЬ ВЫБОР РЕЖИМА «Только прокси» В ИНТЕРФЕЙС**

Найдено ревью задачи 3b: `CaptureMode.proxyOnly` существует в модели и работает в движке, но
выбрать его пользователю НЕГДЕ — переключатель захвата в настройках знает только два значения.
То есть весь режим сейчас физически недостижим, а вместе с ним и тумблер
`applyRulesInProxyOnly`, который показывается только при нём.

Найди переключатель режима захвата в `settings_screen.dart` и добавь третий вариант с
подписью из ARB. ⚠️ Показывать его ТОЛЬКО на Windows: на Android режим не поддержан, и видимый
пункт, который ничего не делает, — ровно тот класс дефектов, за который в этом проекте платили
дороже всего.

⚠️ Проверь, что при выборе `proxyOnly` пропадают контролы, которые в нём не действуют:
kill switch (у него уже есть геттер `AppSettings.killSwitchApplies`) и настройки TUN.
