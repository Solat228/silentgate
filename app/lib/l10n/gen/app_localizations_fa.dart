...
class AppLocalizationsFa extends AppLocalizations {
  ...
  String get tunLogLevelDebugCost =>
      'در سطح debug هسته صدها خط در ثانیه می‌نویسد: گزارش چند دقیقه را پوشش می‌دهد نه چند ساعت و مگابایت‌ها بزرگ می‌شود. برای قطعی‌ای که پیش‌تر رخ داده بی‌فایده است — گزارش برنامه را ببینید.';
  ...
  String get serviceChecksInfo =>
      'دو نماد برای هر سرویس\n• چپ: سرویس بدون VPN\n• راست: همان سرویس از طریق VPN\n• پیکان میان آن‌ها: گذار\n\nزمان انجام سنجش\n• بدون VPN: خودکار هنگام اجرای برنامه\n• از طریق VPN: بلافاصله پس از اتصال\n\nرنگ حلقهٔ دور نماد\n• سبز: باز می‌شود\n• نارنجی: مسدودی کشوری\n• قرمز: در دسترس نیست\n• خاکستری: هنوز بررسی نشده\n\nنشان گوشه همان رنگ را تکرار می‌کند\n• ✓ باز می‌شود\n• ! مسدودی کشوری\n• ✕ در دسترس نیست\n\nچه چیزی را می‌توان زد\n• نماد: بررسی دوباره سرویس\n• دکمهٔ تنظیمات کنار آن: تغییر مجموعهٔ سرویس‌ها';
  ...
  String get noRealIpSubRulesOnly =>
      'فقط قوانین «مستقیم» شما را بازنویسی می‌کند: آن‌ها از تونل عبور می‌کنند (سایت‌های RU هم). مسیر پیش‌فرض را تغییر نمی‌دهد و شبکهٔ محلی مستقیم می‌ماند.';

  @override
  String get noRealIpOnlySelectedNote =>
      'در حالت «فقط موارد انتخاب‌شده»، هر چیزی که انتخاب نشده باشد همچنان با IP واقعی شما خارج می‌شود — این تنظیم آن را تغییر نمی‌دهد.';

  @override
  String get infoNoRealIp =>
      'فقط روی قوانین صریح «مستقیم» (برنامه‌ها و سایت‌ها) و قوانین مستقیمِ پیکربندی پنل اثر دارد: آن‌ها به تونل منتقل می‌شوند. قانونی که گزینهٔ «اجازهٔ IP واقعی» در آن فعال است مستقیم می‌ماند.\n\nکاری که انجام نمی‌دهد: مسیر پیش‌فرض را تغییر نمی‌دهد. در حالت «فقط موارد انتخاب‌شده» مسیر پیش‌فرض مستقیم می‌ماند، بنابراین همهٔ ترافیک انتخاب‌نشده صرف‌نظر از این تنظیم با نشانی واقعی شما خارج می‌شود. اگر باید همه‌چیز پوشش داده شود، از حالت «همه از طریق VPN» استفاده کنید.\n\nشبکهٔ محلی همیشه مستقیم می‌ماند.';

  @override
  String get killSwitchSubProxyNoAdmin =>
      'این یک مسدودسازی واقعی نیست: در حالت «پروکسی سیستم» دسترسی مدیر گرفته نمی‌شود و تنها کاری که انجام می‌شود این است که پروکسی ثبت‌شده باقی می‌ماند. برنامه‌هایی که آن را نادیده می‌گیرند و همهٔ ترافیک UDP مستقیم خارج می‌شوند. فقط حالت TUN همه‌چیز را نگه می‌دارد.';

  @override
  String get killSwitchOfferTun =>
      'مسدودسازی کامل در زمان قطعی فقط در حالت TUN وجود دارد.';

  @override
  String get splitOnlySelectedWarnTitle =>
      'هر چیزی که انتخاب نشده با IP واقعی شما خارج می‌شود';

  @override
  String get splitOnlySelectedWarnBody =>
      'فقط مواردی که کنش «تونل» دارند وارد تونل می‌شوند. بقیهٔ ترافیک — از جمله برنامه‌هایی که از آن‌ها خبر ندارید — مستقیم و با نشانی واقعی شما خارج می‌شود. اگر باید کل دستگاه پنهان بماند، «همه از طریق VPN» را انتخاب کنید.';

  @override
  String get splitOnlySelectedNoRealIp =>
      'تنظیم «با IP واقعی خارج نشو» این را تغییر نمی‌دهد: فقط قوانین «مستقیم» را بازنویسی می‌کند، نه مسیر پیش‌فرض را.';
