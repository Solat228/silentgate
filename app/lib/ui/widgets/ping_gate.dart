...
///
/// ⚠️ ЧТО ГЕЙТ ЗАКРЫВАЕТ — ВСЕ ЧЕТЫРЕ ТОЧКИ ВХОДА ИНТЕРФЕЙСА:
///  * кнопку пинга в шапке `ui/servers_screen.dart`;
///  * пункт «Пинг» в контекстном меню `ui/widgets/server_tile.dart`;
///  * кнопку «Пинг серверов» на главном экране `ui/home_screen.dart`;
///  * пункт «Пинг серверов» в `ui/widgets/subscription_switcher.dart`.
///
/// ⚠️ ДВЕ ПОСЛЕДНИЕ ПЕРЕВЕДЕНЫ ПОЗЖЕ ОСТАЛЬНЫХ, и жили они всё это время с
/// собственным условием `running || speedRunning || пусто` — без проверки
/// автопрогона сервисов, который исполнитель тоже считает запретом. То есть
/// самая заметная кнопка приложения выглядела живой и молча не делала ничего
/// ровно в том случае, на который владелец и жаловался (14 включённых
/// сервисов). Что точки входа действительно идут через гейт, стережёт
/// `test/ping_gate_test.dart` — комментарий тут не доказательство.
///
/// ⚠️ ОДНО МЕСТО СЮДА НЕ ПЕРЕВЕДЕНО СОЗНАТЕЛЬНО: `POST /v1/ping` локального API
/// (`state/api_handlers.dart`). Интерфейса у него нет вовсе — объяснять некому,
/// и уведомление показывать некуда; отказ он возвращает кодом ответа.
///
