...
/// ⚠️ ГДЕ ПРОХОДИТ ЛИНИЯ РАЗДЕЛА — ПОКАЗАНО КАРТИНКОЙ (03.09.2026).
///
/// Разрез идёт ИЗ УГЛА В УГОЛ: из левого нижнего в правый верхний. Обе
/// половины равны: верхний левый треугольник — страна входа, нижний правый — выхода.
///
/// ⚠️ ДВЕ НЕУДАЧНЫЕ ПОПЫТКИ — чтобы не повторить. Владелец дважды
/// просил «больший угол», и доля дважды УМЕНЬШАЛАСЬ (0.75, потом 0.5) по
/// расчёту «чем меньше доля, тем круче линия». Круче — да, но верхняя
/// точка при этом уезжает от правого угла к середине, и верхний флаг
/// вырождается в клин: на снимке от российского флага осталась полоска.
/// Просили НЕ крутизну, а честную половину — то есть долю 1.0.
///
/// ⚠️ ДОЛЯ, А НЕ ПИКСЕЛИ: ячейка рисуется в разных размерах.
const double splitTopFraction = 1.0;
