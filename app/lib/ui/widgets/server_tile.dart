  ...
  Future<void> _menu(BuildContext context, Offset pos) async {
    ...
    switch (action) {
      ...
      case 'speed':
        ...
        //
        // ⚠️ ЗАНЯТОСТЬ ИСПОЛНИТЕЛЯ ЗДЕСЬ БОЛЬШЕ НЕ ЗАПРЕТ, И ЭТО НЕ
        // ПОСЛАБЛЕНИЕ. Раньше стояло `if (probe.running || probe.speedRunning)
        // break;` — молча, без единого слова человеку: массовый замер сотни
        // серверов идёт десятки минут, и всё это время пункт меню выглядел
        // живым и не делал ничего. Требование владельца 02.09.2026 — «в любом
        // случае измерить»: исполнитель сам ставит ручной замер в ГОЛОВУ
        // очереди (поверх массового прогона) либо откладывает до конца пинга.
        // Повторять здесь его прежние условия значит снова обещать одно, а
        // делать другое.
