          ...
          child: Padding(
            ...
            child: _MaybeScroll(
              ...
              child: Column(
                ...
                // ⚠️ FLEXIBLE, А НЕ ПРОСТО ВИДЖЕТ — ИНАЧЕ СЖАТИЕ НЕ ВКЛЮЧАЕТСЯ.
                //
                // Жалоба владельца 03.09.2026 по полному снимку окна: при
                // четырнадцати сервисах блока с информацией о подключении
                // внизу НЕ ВИДНО ВООБЩЕ, при шести срезаны счётчики трафика.
                //
                // Причина не в самом блоке проверок: он умеет ужиматься по
                // высоте (`ServiceChecksSides._estimateScale`), но в `Column`
                // ребёнок получает НЕОГРАНИЧЕННУЮ высоту по главной оси, и
                // `BoxConstraints.hasBoundedHeight` там ложно. То есть сжатие
                // честно отключалось само — «сжимать не от чего», — а блок
                // просто вылезал за окно, выдавливая всё, что под ним.
                //
                // `Flexible` с `FlexFit.loose` отдаёт ему ОСТАТОК высоты после
                // остальных детей: меньше своего размера он не попросит, а
                // больше отведённого не получит — и ужмётся ровно настолько,
                // насколько надо. Решение владельца дословно: «если групп
                // много, делай их пропорционально меньше и используй свободное
                // место по максимуму, но без фанатизма» — «без фанатизма»
                // держит `_minReadableScale`: ниже порога читаемости колонки
                // уступают место привычным рядам, а не мельчают в пыль.
                Flexible(
