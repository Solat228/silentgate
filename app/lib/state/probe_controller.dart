...
class ProbeController extends ChangeNotifier {
  ...
  /// Откуда взять «всё, что приложение знает сейчас».
  ///
  /// ⚠️ ПОСТАВЩИК, А НЕ ГОТОВЫЙ СПИСОК. Собрать его дорого: это разбор сотни
  /// ссылок и декодирование панельных конфигов, и делать это ради ответа
  /// «чистить нечего» нельзя. Ставится связкой в `state/provider_wiring.dart`.
  Iterable<String> Function()? knownServerKeys;

  /// Изменился ли состав серверов с прошлой чистки.
  ///
  /// ⚠️ НАЧАЛЬНОЕ ЗНАЧЕНИЕ — `true`, и это не мелочь: файл пометок переживает
  /// перезапуск, а состав подписки за это время мог поменяться. Без первой
  /// чистки на старте вернувшийся узел пометил бы подписку неполной по
  /// прогону, которого в этой её жизни не было.
  var _prunePending = true;

  /// Состав серверов поменялся — при следующей возможности почистить пометки.
  void markServersChanged() => _prunePending = true;

  /// Почистить пометки, если состав менялся и поставщик списка уже известен.
  ///
  /// ⚠️ ФЛАГ СНИМАЕТСЯ ТОЛЬКО КОГДА ЧИСТКА РЕАЛЬНО ВОЗМОЖНА. Поставщика может
  /// ещё не быть — связка строится позже первого пересчёта списка; сними мы
  /// флаг здесь, чистка не случилась бы вовсе до следующей правки подписки.
  /// Ровно тот класс гонки, на котором в 1.4.3 терялись профили «Авто».
  Future<void> pruneUnfinished() async {
    if (!_prunePending) return;
    // ⚠️ Через ПУБЛИЧНЫЙ геттер, а не через поле: его же читает счётчик
    // подписки, и подменённый в тесте контроллер иначе оказался бы для этой
    // проверки невидим — страж проверял бы код, которого в приложении нет.
    if (unfinishedKeys.isEmpty) {
      _prunePending = false;
      return;
    }
    final supplier = knownServerKeys;
    if (supplier == null) return;
    _prunePending = false;
    await forgetUnknownServers(supplier());
  }
