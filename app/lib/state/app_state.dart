...
class AppState extends ChangeNotifier {
  ...
  AppState({
    ...
    _noticeSub = _engine.notices.listen((n) {
      ...
      // ⚠️ И СИСТЕМНОЕ УВЕДОМЛЕНИЕ ТОЖЕ — иначе его не увидят НА ОБЕИХ
      // платформах.
      //
      // Карточка внутри приложения видна только при открытом окне. На Windows
      // оно почти всегда свёрнуто в трей: 10.08.2026 проверка канала честно
      // отработала и переподняла туннель, карточка показалась — а владелец её
      // не увидел вовсе. На Android приложение свёрнуто не «почти всегда», а
      // всегда, и до 20.08.2026 обрыв там не показывался НИГДЕ (`DesktopNotice`
      // на не-Windows просто писал в журнал). Ветка эта была платформенно
      // нейтральной и тогда — дыра сидела на шаг ниже.
      //
      // Показываем только то, что требует внимания: обычные заметки остаются
      // внутри приложения, чтобы их не начали отмахивать не читая.
      if (n.isProblem) {
  ...
  Future<void> handleIncomingUrl(String url) async {
    ...
    if (action != null) {
      ...
      // ⚠️ «ПОКАЗАТЬ ОКНО» РАЗБИРАЕТСЯ ПЕРВЫМ И НИЧЕГО НЕ МЕНЯЕТ.
      //
      // Это единственное действие, у которого нет ни сервера, ни состояния:
      // его шлёт наш же второй экземпляр, когда человек запустил `.exe`, а
      // приложение уже работает и свёрнуто в трей. Раньше в этом случае не
      // пересылалось ничего, и повторный запуск выглядел как сломанный ярлык.
      if (action == 'show') {
        // Гейт по платформе — как у соседних обращений к трею: окна есть
        // только на Windows, и на Android эта команда прийти не может
        // (второй экземпляр там не запускается).
        if (Platform.isWindows) await TrayWindow.showAndFocus();
        return;
      }
  ...
  /// Состав списка серверов пересобран.
  ///
  /// ⚠️ СУЩЕСТВУЕТ РАДИ ЧИСТКИ ПОМЕТОК «прогон сюда не дошёл». Раньше её звал
  /// переключатель подписок при открытии меню — а он рисуется ТОЛЬКО при двух
  /// и более подписках. У владельца одной подписки (то есть у большинства)
  /// `ping_unfinished.json` не чистился никогда: рос без предела, и
  /// вернувшийся с тем же ключом узел помечал подписку неполной без причины.
  ///
  /// ⚠️ Хук, а не прямой вызов контроллера проб: пробы состоянию не
  /// принадлежат, и лезть за чужим контроллером отсюда — прямой путь к двум
  /// источникам правды. Связка живёт в `state/provider_wiring.dart`.
  void Function()? onServersChanged;
  ...
  void _rebuild() {
    ...
    _rebuildOwnerIndex();
    // ⚠️ ПО ТОЙ ЖЕ ПРИЧИНЕ — И ЗДЕСЬ. Состав серверов с этой строки окончателен,
    // а ниже метод умеет выйти рано: поставь хук в конце, и на самом частом
    // пути о смене состава не узнал бы никто.
    onServersChanged?.call();
  ...
  Future<void> _reconnectWith(
    ...
    // ⚠️ СПРАШИВАЕМ ДВИЖОК, А НЕ ТОЛЬКО ГАЛОЧКУ. Режим захвата может смениться
    // прямо здесь: «Способ захвата» — тоже причина переподключения. Удерживать
    // TUN, когда следующая сессия идёт «Только прокси», значит оставить машину
    // в туннеле ровно в том режиме, который обещает обратное.
    final soft =
        settings.seamlessServerSwitch && _engine.canKeepCaptureFor(settings);
    if (soft) {
    ...
    await connectFn();
    // ⚠️ ПОДКЛЮЧЕНИЕ МОГЛО НЕ НАЧАТЬСЯ ВОВСЕ — а захват мы удержали.
    //
    // `connectFn` выходит рано, если подключаться не к чему (список серверов
    // опустел после переключения подписки). Тогда движок стоит в «отключено» с
    // ЖИВЫМ туннелем, у машины пропадает интернет целиком, а кнопка «Отключить»
    // ничего не делает: `disconnect()` ниже гейтится тем же статусом. Лечится
    // здесь, где ещё известно, что захват удерживали.
    if (soft && !_status.isConnected &&
        _status.state != VpnConnectionState.connecting) {
      AppLog.w('Подключение после смены не началось — снимаю удержанный захват, '
          'иначе туннель остался бы жить без единого ядра');
