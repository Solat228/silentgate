...
class AppState extends ChangeNotifier {
  ...
  Future<void> _reconnectWith(
    ...
    // ⚠️ ВРЕМЕННО ОТКЛЮЧЕНО, И ВОТ ПОЧЕМУ (BACKLOG #30).
    //
    // `_engine.disconnectKeepingCapture()` работает как задумано: живой прогон
    // 18.08.2026 показал, что туннель ВПЕРВЫЕ сохраняется — после смены сервера
    // в журнале нет ни строки «TUN автоподбор». Но всплыл упор, которого не
    // было видно раньше: порт «Прямо» (10819) принадлежит ТОМУ САМОМУ
    // TUN-процессу sing-box, который мы намеренно оставляем жить. Сохранили
    // туннель — сохранили и порт, а новая сессия проверяет его как свободный и
    // падает: «Порт 10819 ещё занят нашим ядром от прошлой сессии».
    //
    // Лечение известно и невелико: при переиспользовании туннеля исключить из
    // проверки порты, которыми владеет сам переиспользуемый туннель, — они наши
    // и никуда не уходят. Но это правка пути подключения, и включать её без
    // полного прогона в VM нельзя: цена ошибки здесь — сборка, которая не
    // поднимает VPN.
    await disconnect();
    await connectFn();
