...
class SingboxRouterWindows implements TunRouter {
  ...
  Future<void> _startOnce(SplitTunnelConfig split, int xraySocksPort,
    ...
    if (!await TunHelper.waitStopConsumed(_stopPath, abort: abort)) {
    ...
    // ⚠️ ПОСЛЕДНЯЯ ТОЧКА, ГДЕ ОТМЕНА ЕЩЁ ДЁШЕВА. Дальше идёт запуск помощника с
    // правами администратора: он поднимет туннель, и отменять придётся уже
    // снятием поднятого. Между ожиданием stop-файла (до 5 с) и этим местом
    // человек вполне мог нажать «Отключить».
    if (abort?.call() ?? false) {
      AppLog.i('TUN: отключение пользователем — помощник не запускается');
      return;
    }
