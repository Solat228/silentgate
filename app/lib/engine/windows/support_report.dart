...
class SupportReport {
  ...
  static Future<String> generate({
    ...
    // ⚠️ МАСКА НА ВЕСЬ ОТЧЁТ, А НЕ ПО СЕКЦИЯМ — ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ОН
    // СТАНОВИТСЯ ФАЙЛОМ. Ревью нашло, что шапка печатает «Выбран: <сервер>», а
    // это `displayName`: у безымянного узла — его боевой адрес, и собирается
    // строка в интерфейсе, мимо очистки журнала. Утечка жила на шестьдесят
    // строк ВЫШЕ уже замаскированного `app.log` — и называла ровно тот узел, о
    // котором пойдёт разговор в чате поддержки. Чинить по одной строке здесь
    // бессмысленно: следующая секция снова обойдёт очистку, как обошли эта и
    // логи ядер до неё. Маска идемпотентна, поэтому повторный проход по уже
    // очищенным секциям ничего не портит.
