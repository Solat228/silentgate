...
class WindowsEngine extends VpnEngineBase {
  ...
  /// Поднятый TUN затягивает сокеты самого приложения — TCP-пинг с этого
  /// момента меряет локальный стек sing-box, а не серверы (1–3 мс у всех
  /// подряд, включая мёртвые; `route_exclude_address` не спасает — проверено
  /// живым замером). Отдаём пингу Clash API туннельного экземпляра: по нему
  /// текущий сервер меряется самим ядром, а остальные честно помечаются.
  ///
  /// Секрет читается ЗДЕСЬ, а не кэшируется потребителем: при бесшовной
  /// смене сервера поле возвращается к секрету живого туннеля (см. `raiseTun`),
  /// и снимок, сделанный раньше, стучался бы с чужим паролем.
  @override
  LiveCoreApi? get captureCoreApi => _tunActive
      ? LiveCoreApi(port: _tunApiPort, secret: singboxApiSecret)
      : null;
  ...
  Future<void> startSession() async {
    ...
    try {
      ...
      if (wantProxy) {
        ...
        // внутри `set()`.
        //
        // ⚠️ РЕЗУЛЬТАТ ОБЯЗАН БЫТЬ ИСПОЛЬЗОВАН. `await` без присваивания —
        // законный Dart, и ни компилятор, ни анализатор про него не скажут:
        // ровно так отказ и жил незамеченным. Журнала мало — человек читает
        // его уже ПОСЛЕ того, как поработал под реальным адресом.
        final proxyOk = await SystemProxy.set('127.0.0.1:${ports.http}');
        if (!proxyOk) {
          emitNotice(
              EngineNoticeKind.systemProxyFailed,
              'Системный прокси не включился — трафик идёт мимо VPN, '
              'под вашим реальным адресом',
              detail: 'Чаще всего это запрет записи в реестр (политика домена '
                  'или повреждённый профиль). Подробности — в журнале '
                  'приложения. Обходной путь: режим захвата «TUN».');
        }
        _proxySet = true;
        _proxyOwnerGen = gen;
        if (options.captureMode == CaptureMode.tun) {
  ...
  Future<bool> raiseTun({
    ...
    // ⚠️ ЗАПАСНОЙ ПУТЬ ВЫХОДА: умер выход — трафик правила уходит в общий
    // туннель, а не в никуда. До этого отказ выхода означал, что правило
    // просто перестаёт работать, и человек видел не «выход умер», а
    // «сайт не открывается».
    //
    // ⚠️ Страна при этом меняется — трафик пойдёт через основной сервер
    // сессии. Это осознанный размен: рабочее соединение через другую
    // страну полезнее неработающего правила, и сторож выходов говорит о
    // переключении вслух.
    fallbackTag: 'proxy',
    );
    ...
    final tunOptions = TunOptions.fromSettings(
      ...
      directFallbackDnsPort:
          await _startDirectFallbackDns(options.settings, gen, aborted),
  ...
  Future<SingboxProcess?> _startExitRouter(
    ...
    try {
      ...
      // ⚠️ ЗДЕСЬ ЗАПАСНОГО ПУТИ НЕТ НАМЕРЕННО, И ЭТО НЕ ЗАБЫТАЯ ПРАВКА.
      //
      // Выше, для выходов правил, запасной тег передаётся: умер выход —
      // трафик уходит в общий туннель, потому что рабочее соединение через
      // другую страну полезнее неработающего правила.
      //
      // Порты локального API — противоположный случай. Скрипт обращается к
      // конкретному номеру порта именно потому, что ему нужен КОНКРЕТНЫЙ
      // сервер («дай мне Германию»). Подменить его общим туннелем значит
      // молча отдать скрипту другую страну — он об этом не узнает и продолжит
      // считать, что вышел откуда просил. Честный отказ соединения здесь
      // полезнее тихой подмены.
      final exitsBuilt = ExitOutbounds.build(
        servers: options.exitServers,
        resolvedIps: VpnEngineBase.pickOneIpPerHost(exitHosts),
      );
  ...
  Future<int> _startDirectFallbackDns(
      AppSettings s, int gen, bool Function() aborted) async {
    // ⚠️ ПОД KILL SWITCH ПРЯМОГО ЗАПАСА БЫТЬ НЕ ДОЛЖНО — ЭТО ДЫРА В СВОЕЙ ЖЕ
    // ЗАЩИТЕ.
    //
    // Kill switch закрывает 53-й порт мимо туннеля правилом `blockDns` именно
    // затем, чтобы провайдер не видел список доменов. Но наши собственные
    // бинари разрешены ПРАВИЛОМ ВЫШЕ, и форвардер — это `silentgate.exe`.
    // Значит, подняв его, мы бы аккуратно вынесли весь прямой DNS наружу
    // через дырку, оставленную для себя, и защита стала бы декоративной.
    //
    // ⚠️ Оговорка была записана ЗАРАНЕЕ (память `rejected-approaches`, разбор
    // DNS-фолбэка) — «только для «Прямо», только по молчанию, не при
    // killSwitch/noRealIp» — и я её всё равно потерял при реализации. Нашлось
    // случайно, при сжатии памяти. Здесь она в коде, а не в заметке.
    if (s.killSwitch || s.noRealIp) {
      AppLog.i('Прямой запас DNS не поднимаю: включён '
          '${s.killSwitch ? "kill switch" : "режим без реального IP"} — прямой резолв в обход туннеля противоречил бы ему');
      return 0;
    }
