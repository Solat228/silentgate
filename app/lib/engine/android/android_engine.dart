...
class AndroidEngine extends VpnEngineBase {
  ...
  /// Файл с кредами инбаунда проб — рядом с паролем Clash API.
  ///
  /// ⚠️ ДА, ЭТО ЗАПИСЬ СЕКРЕТА НА ДИСК, И ОНА ОБОСНОВАНА ТЕМ ЖЕ, ЧЕМ СОСЕДНЯЯ.
  /// Каталог данных приложения на Android другим приложениям недоступен —
  /// защита, ради которой пароль и заведён (loopback между приложениями не
  /// изолирован), от этого не слабеет. А альтернатива хуже: без записи новый
  /// запуск интерфейса, подхвативший живой туннель, не может опросить его
  /// проб-порт — и сторож канала либо не вооружается вовсе (обрыв тогда некому
  /// заметить), либо вооружается без кредов и получает 407, то есть объявляет
  /// ИСПРАВНЫЙ туннель мёртвым.
  static final String _NL = String.fromCharCode(10);

  static Future<File> _probeSecretFile() async {
    final dir = await AppPaths.supportDir();
    return File('${dir.path}${Platform.pathSeparator}probe_secret');
  }

  /// Сохранить креды проб для будущего подхвата. Формат — две строки.
  Future<void> _saveProbeSecret() async {
    try {
      await (await _probeSecretFile())
          .writeAsString('${ProxyProbe.user}$_NL${ProxyProbe.password}',
              flush: true);
    } catch (e) {
      // Не фатально: подключение важнее сторожа. Потеряется только его
      // вооружение после возврата в приложение — и об этом будет строка.
      AppLog.w('Не удалось сохранить пароль проб: $e');
    }
  }

  /// Креды проб туннеля, поднятого прошлым запуском интерфейса.
  Future<bool> _restoreProbeSecret() async {
    try {
      final f = await _probeSecretFile();
      if (!await f.exists()) return false;
      final parts = (await f.readAsString()).split(_NL);
      if (parts.length < 2) return false;
      final user = parts[0].trim();
      final pass = parts[1].trim();
      // ⚠️ Пустой пароль хуже отсутствующего: проба пошла бы без кредов и
      // получила 407, то есть объявила бы исправный туннель мёртвым.
      if (user.isEmpty || pass.isEmpty) return false;
      ProxyProbe.user = user;
      ProxyProbe.password = pass;
      return true;
    } catch (e) {
      AppLog.w('Не удалось прочитать пароль проб: $e');
      return false;
    }
  }
  ...
  Future<void> adoptRunningTunnel() async {
    try {
      ...
      // ⚠️ СТОРОЖ КАНАЛА ВООРУЖАЕТСЯ И ЗДЕСЬ, А НЕ ТОЛЬКО ПРИ СВОЁМ ПОДЪЁМЕ.
      //
      // Без этого самый обычный сценарий Android оставался без присмотра:
      // человек смахнул приложение, туннель продолжил работать, человек
      // вернулся — и обрыв канала больше некому заметить. «Подключено» на
      // экране висело бы при мёртвом туннеле сколько угодно долго.
      //
      // ⚠️ Но только с кредами проб. Без них проба получает 407 и объявляет
      // ИСПРАВНЫЙ туннель мёртвым — сторож стал бы источником обрывов, а не
      // защитой от них. Поэтому «не нашли пароль» = не вооружаемся, и причина
      // называется вслух: молчание тут неотличимо от исправной работы.
      if (await _restoreProbeSecret()) {
        // ⚠️ ПОКОЛЕНИЕ БЕРЁМ ЗДЕСЬ И СВЕРЯЕМСЯ С НИМ. Сторож обязан замолчать,
        // когда человек отключится или переподключится: без этого он остался бы
        // сторожем ЧУЖОЙ сессии и глушил бы новую (тот же разбор, что в
        // `startHealthWatch`).
        final gen = newGeneration();
        startHealthWatch(() => isStale(gen));
      } else {
        AppLog.w('Сторож канала НЕ вооружён после подхвата: пароль проб '
            'прошлой сессии не найден. Обрыв этого туннеля замечен не будет — '
            'переподключитесь, чтобы вернуть присмотр.');
      }
    } catch (e) {
  ...
  Future<void> startSession() async {
    ...
    // Кладём рядом с данными: следующий запуск интерфейса подхватит этот
    // туннель и должен уметь опросить его проб-порт, иначе сторож канала не
    // вооружится и обрыв станет некому заметить.
    await _saveProbeSecret();
    ...
    try {
      ...
      // обычный сервер, правка пользователя и панельный профиль «Авто» — Xray;
      // sing-box сам поднимает только hysteria2.
      ...
      // ⚠️ Признак «поднимать Xray» — ЯДРО, которое выбрала база, и только оно.
      // Всё, что Xray-ядро (обычный VLESS/Trojan/SS, панельный профиль «Авто»,
      // правка из JSON-редактора), идёт через libxray и SOCKS-мост в sing-box —
      // РОВНО как на Windows. hysteria2 (`core == singbox`) Xray не умеет, и она
      // остаётся на sing-box.
      //
      // ⚠️ ПОЧЕМУ ОБЫЧНЫЙ СЕРВЕР БОЛЬШЕ НЕ ВСТРАИВАЕТСЯ В SING-BOX НАПРЯМУЮ.
      // Кто делает REALITY-рукопожатие — тот и представляется серверу. Xray на
      // ноде отбрасывает в fallback клиента, назвавшегося версией ниже
      // `minClientVer` (умолчание после апгрейда нод — 26.3.27), и делает это
      // МОЛЧА: ни строки в журнале ни у нас, ни у ноды, туннель «поднят»,
      // трафика нет. sing-box представляется хардкодом 1.8.1 — режется всегда.
      // До 11.09.2026 обычный сервер на Android шёл прямо через sing-box и
      // держался ТОЛЬКО на чужой настройке `minClientVer: 0.0.0` на нодах:
      // сняли бы её — Android отвалился бы без единого признака. Владелец решил
      // от зонтика не зависеть: REALITY-клиентом везде должен быть наш Xray
      // 26.3.27. Windows так работает с самого начала — там сомнений не было.
      //
      // ⚠️ ЦЕНА — ЛИШНИЙ ПЕРЕХОД sing-box → SOCKS 127.0.0.1:10808 → libxray на
      // телефоне: два Go-рантайма в процессе, каждый пакет проходит два стека,
      // ещё одно локальное соединение на каждое исходящее. От этого перехода
      // уходили СОЗНАТЕЛЬНО (ради батареи и простоты) — и возвращаем его
      // сознательно: молчаливый fallback хуже любого расхода. Если понадобится
      // мерить цену — сравнивать `batterystats` за равные интервалы на ОДНОМ
      // сервере до/после, а не «на глаз».
      //
      // История, чтобы не ходить по кругу. Первое условие здесь было
      // `multi || !supports(first)` и ВЫБРАСЫВАЛО панельный профиль и правку
      // из JSON для любого сервера, чей протокол умеет sing-box (то есть для
      // всех Remnawave: `protocol` профиля берётся с первого outbound-а и равен
      // vless). Второе — `cfg.full || !supports(first)` — это чинило, но
      // обычный сервер оставляло на sing-box. Третье, нынешнее, — расширяет
      // путь через Xray на всё, что база собрала для Xray. Страж —
      // `test/android_engine_xray_path_test.dart`: он читает не эту
      // переменную, а `xray_config` в аргументах `start` — единственный
      // признак, по которому Kotlin поднимает libxray.
      final viaXray = cfg.core == ProxyCore.xray;

      // Когда сервер поднимает Xray, туннель заворачивает трафик в его
      // локальный SOCKS — как на Windows. Только когда ядро — sing-box
      // (hysteria2), outbound встраивается прямо в конфиг туннеля.
  ...
  ///
  /// [xrayJson] == null — Xray в этой сессии не поднимается (hysteria2:
  /// её целиком обслуживает sing-box; обычные серверы с 11.09.2026 идут через
  /// Xray, см. `viaXray` в [startSession]).
  @visibleForTesting
  ...
  Future<int> startFallbackDns(AppSettings s,
    ...
    //    обычном сервере подписки — где Xray ТОГДА не поднимался вовсе (с
    //    11.09.2026 поднимается; без Xray сегодня только hysteria2) и SOCKS
    //    10808 некому было слушать — форвардер всё-таки поднимался. Каждый
    //    запрос стучался
