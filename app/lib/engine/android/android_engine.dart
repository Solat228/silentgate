...
class AndroidEngine extends VpnEngineBase {
  ...
  Future<void> startSession() async {
    ...
    try {
      ...
      if (aborted()) return;
      // ⚠️ ПРИ ВКЛЮЧЁННОЙ БЕСШОВНОСТИ СЮДА ИДУТ И ЗАПАСНЫЕ СЕРВЕРЫ. Тогда конфиг
      // туннеля перестаёт зависеть от того, кто из кандидатов выбран, и
      // повторный `start` с тем же конфигом не пересоздаёт интерфейс: нативная
      // сторона перезагружает ядра на месте, а `VpnService` оставляет тот же fd
      // (ровно на этом уже держится заглушка kill switch — она обязана совпасть
      // с живым туннелем поле в поле, иначе система пересоздаёт интерфейс).
      // Выключенный флаг отдаёт РОВНО прежний список.
      final serverIps =
          await tunnelBypassIps(hosts, session.options.settings);
  ...
  Future<void> teardownCore({bool keepCapture = false}) async {
    ...
    // ⚠️ ЗАХВАТ УДЕРЖИВАЮТ ДВЕ РАЗНЫЕ НАСТРОЙКИ, И ГОВОРИТЬ О НИХ НАДО РАЗНОЕ.
    // Kill switch обещает блокировку и объявляет её вслух (уведомление в
    // шторке); «не мигать сетью» (`seamlessKeepTun`) ничего не блокирует
    // намеренно — он лишь не даёт пересоздавать интерфейс между попытками.
    // Показать в этом случае «трафик заблокирован» значило бы пообещать защиту,
    // которую пользователь не включал, и человек, поверив, ушёл бы работать
    // мимо VPN спокойным.
    final asKillSwitch = session?.options.settings.killSwitch ?? false;
    if (keepCapture) {
      ...
      if (stub == null) {
        ...
        AppLog.w('Удержать туннель нечем — параметры живого туннеля '
            'неизвестны. Гашу, чтобы не подменять интерфейс наугад.');
        await _stopService();
        return;
      }
      try {
        ...
        if (asKillSwitch) {
          ...
          // Сказать об этом в шторке. Приложение в этот момент чаще всего
          // закрыто, и другого способа объяснить пропавший интернет у нас нет:
          // без объяснения человек решит, что сломалось, и выключит VPN — то
          // есть сделает ровно то, от чего защита оберегала.
          try {
          ...
          // ⚠️ Честно и без обещаний: пока ядер нет, пакеты в туннель заходят и
          // никуда не выходят — иначе удержать интерфейс на Android нечем
          // (дескриптором владеет libbox). Разница с kill switch в том, что
          // попытки здесь КОНЧАЮТСЯ, и после них `cleanup()` снимет туннель.
          AppLog.i('Туннель удержан между попытками, чтобы не пересоздавать '
              'интерфейс; блокировка при этом НЕ включается — попытки конечны');
        }
        return;
      } catch (e) {
        // Не смогли удержать — честнее погасить, чем оставить неизвестное
        // состояние: иначе туннель мог бы жить со СТАРЫМ конфигом.
        AppLog.w('Не удалось удержать туннель ($e), гашу');
      }
    }
    await _stopService();
  }
