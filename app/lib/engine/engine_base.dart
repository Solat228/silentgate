  ...
  void startExitHealth({
    ...
    final h = ExitHealth(
      ...
      mainChannelAlive: () {
        ...
        // ⚠️ ОСТАНОВЛЕННЫЙ СТОРОЖ = «НЕ ЗНАЮ», А НЕ «КАНАЛ МЁРТВ».
        //
        // Найдено ревью 05.09.2026. `TunnelHealth` при приговоре гасит свой
        // таймер и НЕ обнуляет счётчик промахов — тот остаётся равным трём
        // навсегда. А предохранитель движка (три переподъёма подряд не
        // помогли) возвращается, НЕ вооружая сторож заново и не трогая
        // соединение. С этой секунды гейт закрыт до конца сессии: сторож
        // выходов выходит на первой строке каждые 60 с, `exitHealthArmed`
        // при этом продолжает отвечать «вооружён», и в журнале — ни строки.
        //
        // То есть в единственном случае, ради которого сторож выходов и
        // написан (канал жив, а конкретный выход умер), он оказывался молча
        // выключен. Спрашиваем ФАКТ работы сторожа, а не его счётчик.
    ...
    h.start(
      ...
      onExitRecovered: (tag, missed) {
        down.remove(tag);
        AppLog.i('Выход «${names[tag] ?? tag}» снова отвечает '
            '(промахов было $missed)');
      },
      onExitDown: (tag) async {
        if (!down.add(tag)) return;
        final name = names[tag] ?? tag;
        AppLog.e('Выход «$name» не отвечает: ${h.failuresToDeclareDown} '
            'проверки подряд не прошли.');

        // ⚠️ ВСЕ ВЫХОДЫ РАЗОМ — ЭТО ОТКАЗ ПРОБЫ, А НЕ ВЫХОДОВ.
        //
        // 03.09.2026 владелец получил ШЕСТЬ заметок подряд, накрывших пол-экрана:
        // «Германия 2.7», «Польша 1.1», «Нидерланды 1.1»… Столько выходов не
        // умирает одновременно — умер общий для них ответчик (Clash API ядра
        // либо сам запрос задержки). Кричать в этом случае про каждый выход
        // значит врать шесть раз вместо одного.
        //
        ...
        if (!tellUserExitDown(down: down.length, total: tags.length)) {
          AppLog.w('Не отвечают ВСЕ выходы (${tags.length} из ${tags.length}) '
              ...
              'ядра. Человеку не сообщаю: одновременная смерть всех выходов '
              'неправдоподобна, и шесть заметок были бы шестью ошибками.');
          return;
        }

        // ⚠️ ТЕКСТ БЕЗ СЛОВА «ПРАВИЛА». Выход поднимается не только под правило
        // раздельного туннелирования, но и под ОТДЕЛЬНЫЙ ПОРТ локального API
        // (`apiExitServerKeys`). У владельца 03.09.2026 таких было большинство,
        // и заметка про «правила, привязанные к серверу» была прямой неправдой:
        // «у меня НЕТ таких правил».
        // ⚠️ ПЕРЕКЛЮЧАЕМ НА ЗАПАСНОЙ ПУТЬ, А НЕ ТОЛЬКО СООБЩАЕМ.
        //
        // Выход собран группой `selector` из двух участников: сам сервер и
        // общий туннель. Группа сама НИЧЕГО не решает — `selector` не пробует
        // участников (этим и отличается от `urltest`, который на телефоне
        // будил бы радио каждые три минуты). Решение принимает этот сторож, по
        // факту трёх промахов подряд, и отдаёт его командой в Clash API.
        //
        // Без этой строки запас был бы построен и не использован — ровно тот
        // класс бед, что в проекте уже случался трижды: код написан, покрыт
        // тестами, и не вызывается ниоткуда.
        final switched = await stats.selectExitMember(tag, fallbackMemberTag);
        emitNotice(
          EngineNoticeKind.exitDown,
          'Выход «$name» не отвечает',
          detail: switched
              // Про смену страны говорим ПРЯМО. Человек направил этот трафик
              // через конкретный сервер осознанно, и молчаливая подмена
              // страны — это ровно то, чего он не просил.
              ? 'Трафик, направленный через этот сервер, временно идёт через '
                  'основной туннель — значит из другой страны. Вернётся '
                  'обратно, как только сервер снова ответит.'
              : 'Трафик, направленный через этот сервер, сейчас не идёт. '
                  'Основной туннель и остальные выходы работают.',
        );
      },
    );
  }

  /// Участник группы выхода, на который уводим трафик при отказе.
  ///
  /// Совпадает с тем, что кладётся в группу построителем
  /// (`ExitOutbounds.build(fallbackTag: ...)`): общий туннель сессии.
  static const fallbackMemberTag = 'proxy';

  /// Говорить ли человеку о мёртвом выходе.
  ///
  /// ⚠️ ВСЕ ВЫХОДЫ РАЗОМ — ЭТО ОТКАЗ ПРОБЫ, А НЕ ВЫХОДОВ, и молчать тут
  /// честнее. Чистая функция намеренно: правило важнее кода вокруг него, и
  /// проверять его надо отдельно от таймеров и сети.
  @visibleForTesting
  static bool tellUserExitDown({required int down, required int total}) =>
      total <= 1 || down < total;
  void stopExitHealth() {
    _exitHealth?.stop();
    _exitHealth = null;
  ...
  void startHealthWatch(bool Function() aborted) {
    ...
    h.start(
      ...
      onDown: () async {
        ...
        // ⚠️ НАЗЫВАЕМ, ЧТО ИМЕННО СЛОМАЛОСЬ, А НЕ ПРОСТО «НЕ ПРОШЛО».
        //
        // Мишени задаются именами, и резолвит их САМО ЯДРО туннельным DNS.
        // Пока в журнале стояла одна строка на оба случая, «транспорт умер» и
        // «в туннеле сломался DNS» были неразличимы — а лечатся они по-разному.
        // У владельца из-за этого шесть циклов за ночь с периодом ровно в три
        // минуты выглядели одинаково (`docs/BACKLOG.md` #31).
        //
        // Спрашиваем ТОЛЬКО здесь, в момент приговора: платить лишней пробой на
        // каждом такте незачем.
        final byIp = await h.probeOnce();
        final byName = byIp ? await h.nameResolutionWorks() : false;
        final what = !byIp
            ? 'не отвечает ничего — похоже, умер транспорт'
            : (byName
                ? 'на повторе ответило всё — вероятно, помеха была недолгой'
                : 'ОТВЕЧАЕТ ТОЛЬКО МИШЕНЬ ПО АДРЕСУ, по именам — нет: '
                    'транспорт жив, сломан DNS в туннеле');
        AppLog.e('Канал не отвечает: ${h.failuresToDeclareDown} проверки подряд '
            'не прошли через прокси-порт $httpProxyPort ($what). Ядро при этом '
            'живо — переподключаюсь, иначе «Подключено» осталось бы враньём.');
