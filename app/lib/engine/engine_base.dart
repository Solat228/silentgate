...
abstract class VpnEngineBase implements VpnEngine {
  ...
  Future<void> connectWith(String configJson, ConnectionOptions options,
    ...
    _lastUpAt = null;
    // ⚠️ Накопленные адреса «мимо туннеля» принадлежат ПРОШЛОМУ подключению и
    // обязаны уйти вместе с ним: правило выпускает трафик к этим адресам мимо
    // VPN, и сервер удалённой подписки жил бы в нём до перезапуска приложения.
    // Здесь же, а не в `disconnect()`: сюда приходят ВСЕ команды подключения, а
    // отключением не заканчивается, например, перезапуск по правке настроек.
    _bypassIps.clear();
