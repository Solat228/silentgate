...
abstract class VpnEngineBase implements VpnEngine {
  ...
  bool keepCaptureBetweenAttempts(AppSettings s) =>
      s.killSwitch || s.seamlessKeepTun;
  ...
  Future<List<String>> tunnelBypassIps(
    ...
    //
    // ⚠️ Копится РОВНО ОДНО подключение: [connectWith] очищает набор. Иначе
    // адреса прежней подписки остались бы в правиле навсегда, а правило это
    // выпускает трафик к ним МИМО туннеля.
    _bypassIps.addAll(ips);
    return _bypassIps.toList()..sort();
  }

  /// Адреса «мимо туннеля», накопленные за ТЕКУЩЕЕ подключение.
  /// Живёт от команды пользователя до следующей такой команды — см.
  /// [tunnelBypassIps] и очистку в [connectWith].
