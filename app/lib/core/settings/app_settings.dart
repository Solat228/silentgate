...
extension ProbeServiceLabel on ProbeService {
  ...
  /// Есть ли у сервиса ОТДЕЛЬНАЯ проверка доступности в стране выхода.
  ///
  /// ⚠️ GEMINI СЮДА НЕ ВХОДИТ, И ЭТО ИЗМЕРЕНО, А НЕ ЗАБЫТО. `gemini.google.com`
  /// отдаёт одну и ту же страницу всюду — 200 и 805 КБ без единого слова о
  /// стране (замер 19.08.2026 из РФ, где сервис недоступен). Недоступность
  /// Google показывает уже после входа в аккаунт, лёгкой пробой её не увидеть.
  /// Числиться «гео-проверяемым» и не проверять ничего — хуже, чем честно не
  /// числиться: зелёный чип читался бы как «доступен в вашей стране».
  bool get geoGated =>
      this == ProbeService.chatgpt || this == ProbeService.claude;
}
...
class AppSettings {
  ...
  int get speedTestTrafficMb =>
      ((effectiveSpeedTopN + 1) * autoConfigSpeedSize.bytes) ~/ 1000000;

  /// Объём одной пробы В АВТОНАСТРОЙКЕ — всегда лёгкий, независимо от
  /// [speedTestSize].
  ///
  /// ⚠️ ЭТО НЕ ТА ЖЕ ВЕЛИЧИНА, ЧТО НА ЭКРАНЕ СЕРВЕРА, И ПУТАТЬ ИХ НЕЛЬЗЯ.
  /// `speedTestSize` — точность РУЧНОГО замера одного сервера; автонастройка
  /// гоняет несколько подряд и платит трафиком подписки, поэтому `_rankBySpeed`
  /// жёстко берёт лёгкую пробу. Предупреждение считало по `speedTestSize` — и
  /// при выбранных 20 МБ обещало 220 МБ вместо реальных 55. Предупреждение о
  /// расходе, которое врёт вчетверо, хуже отсутствующего: после него человек
  /// перестаёт верить и остальным цифрам. Найдено ревью 18.08.2026.
  static const autoConfigSpeedSize = SpeedTestSize.light;
