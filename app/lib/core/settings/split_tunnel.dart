...
/// Проверять ли путь правила «по полному пути» на существование на диске.
///
/// ⚠️ На Android и iOS в [AppRule.path] лежит ИМЯ ПАКЕТА, а не путь к файлу
/// (`com.android.chrome`) — проверка существования файла там всегда давала бы
/// «не найден» и не значила бы ничего.
bool appRulePathCheckApplies() =>
    Platform.isWindows || Platform.isLinux || Platform.isMacOS;

/// Указывает ли правило «по полному пути» на несуществующий файл.
///
/// ⚠️ ОДНА ФУНКЦИЯ НА ДВУХ ПОТРЕБИТЕЛЕЙ, А НЕ ВТОРАЯ КОПИЯ. Ровно эта проверка
/// нужна и пометке в списке правил (`ui/widgets/dead_path_badge.dart`), и
/// уведомлению при подключении на главном экране
/// ([EngineNoticeKind.deadAppRule] — движок и состояние приложения не
/// импортируют виджеты UI, поэтому проверка живёт здесь, в `core`, а виджет
/// зовёт её же). Второй, разошедшийся с этим, парсер дал бы место, где два
/// способа сказать «правило сломано» расходятся во мнениях.
///
/// Правило «по имени» сюда не попадает: сопоставление от пути не зависит и
/// исправно переживает переезд файла (см. [AppRule.matches]).
bool appRulePathMissing(AppRule r) {
  if (r.byName || !appRulePathCheckApplies() || r.path.trim().isEmpty) {
    return false;
  }
  try {
...
String? _serverKeyFrom(Object? v) {
  ...
  // ⚠️ Ключ приводим к каноническому виду ПРИ ЧТЕНИИ — иначе правило «этот сайт
  // через Германию» молча теряет свой выход. Сервер, ключ которого не нашёлся,
  // трактуется как `null` (общий туннель) БЕЗ ПРЕДУПРЕЖДЕНИЯ: висячий тег в
  // конфиге опаснее, потому что `sing-box check` его не ловит. То есть после
  // смены формата ответа панели пользователь просто обнаруживал бы, что весь
  // трафик пошёл через основной сервер, и никакого следа об этом нет.
  return ShareLinkParser.canonicalKey(s);
}
