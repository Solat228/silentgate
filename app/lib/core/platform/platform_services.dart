...
abstract interface class AppCatalog {
  ...
  /// Готовое человеческое имя из кэша — синхронно, чтобы список не мигал при
  /// прокрутке. `null` — ещё не спрашивали или метки нет.
  String? cachedLabel(String key);

  /// Человеческое имя приложения по ключу правила.
  ///
  /// ⚠️ ЗАЧЕМ ОТДЕЛЬНЫЙ ЗАПРОС, А НЕ ПОЛЕ В ПРАВИЛЕ. Метку соблазнительно
  /// записать в `AppRule` при добавлении, но она попала бы в `toJson`, а
  /// подпись раздельного туннелирования сравнивается целиком и решает, нужно
  /// ли ПЕРЕПОДКЛЮЧАТЬСЯ. Тогда (а) заполнение меток у старых правил рвало бы
  /// живой туннель, (б) смена языка телефона меняла бы метки — и рвала его
  /// снова. Ровно тот класс «беспричинных обрывов», который уже чинили.
  /// Поэтому метка резолвится на лету и живёт только в памяти.
  Future<String?> labelFor(String key);
}
