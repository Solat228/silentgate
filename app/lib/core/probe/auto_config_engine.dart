...
class AutoConfigEngine {
  ...
  Future<List<AutoConfigResult>> _rankBySpeed(
    ...
    // ⚠️ ОДИН ИСТОЧНИК С ПРЕДУПРЕЖДЕНИЕМ О ТРАФИКЕ. Здесь стояла своя
    // константа, а окно «потратит N МБ» считало по `speedTestSize` — цифры
    // расходились вчетверо. Теперь обе стороны спрашивают одно поле.
    const size = AppSettings.autoConfigSpeedSize;
