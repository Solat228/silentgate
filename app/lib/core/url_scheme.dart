...
abstract final class AppUrlScheme {
  ...
  static const controlActions = {
    'connect',
    'disconnect',
    'toggle',
    'update',
    // ⚠️ ПОКАЗАТЬ УЖЕ ОТКРЫТОЕ ПРИЛОЖЕНИЕ. Жалоба владельца 05.09.2026:
    // свёрнутую в трей программу нельзя было вызвать повторным запуском
    // `.exe` — «просто ничего не происходит». Второй экземпляр пересылал
    // первому только ССЫЛКУ, а при двойном клике по ярлыку ссылки нет
    // вовсе: не пересылалось ничего, и процесс тихо завершался.
    //
    // Для человека это выглядит как сломанный ярлык — он жмёт второй и
    // третий раз, и на экране пусто. Трей ради того и делают, чтобы
    // приложение возвращалось; без этой команды он был ловушкой.
    'show',
  };

  /// Ссылка «покажи окно» — то, что второй экземпляр шлёт первому вместо
  /// молчаливого выхода.
  static const showWindowUrl = '$scheme://show';
