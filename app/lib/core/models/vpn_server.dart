...
class VpnServer {
  ...
  String get identityKey {
    ...
    // перетасовывает — узлов там десятки.
    if (isPanelProfile) return 'panel|${remark.trim()}';
    // ⚠️ АДРЕСА И ПОРТА ЗДЕСЬ НЕТ — ТРЕБОВАНИЕ ВЛАДЕЛЬЦА 02.09.2026: «обновление
    // сервера, а не полная замена, если название совпадает, а различаются только
    // некоторые параметры».
    //
    // Раньше тождество было `протокол|адрес|порт|имя`, и панель, поправив
    // серверам адрес, выдавала «+5 · −6» из ОДНИХ И ТЕХ ЖЕ имён: пять
    // «Германия 2.x» стояли в баннере одновременно с плюсом и с минусом.
    // Человек читает это как «мне перетасовали половину подписки», хотя
    // изменился только адрес.
    //
    // ⚠️ ДУБЛИКАТЫ ИМЁН ЛОВИТ НЕ ЭТОТ ГЕТТЕР, А ДИФ: если в одном списке два
    // сервера с одинаковым протоколом и именем, они схлопнулись бы в одну
    // запись и диф решил бы, что один пропал. Разводит их
    // `SubscriptionSyncResult.diff` — см. там `_byIdentity`.
    return '$protocol|${remark.trim()}';
  }
