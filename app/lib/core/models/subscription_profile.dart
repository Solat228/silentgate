...
class SubscriptionProfile {
  ...
  /// Истекла ли подписка к моменту [now] — по дате из СОХРАНЁННЫХ метаданных.
  ///
  /// ⚠️ ЕДИНСТВЕННЫЙ ИСТОЧНИК ДАТЫ ДЛЯ НЕАКТИВНОЙ ПОДПИСКИ. В состоянии
  /// приложения (`AppState.info`) живёт карточка ТОЛЬКО активной подписки, а в
  /// меню переключателя их четыре — у остальных дату взять больше негде.
  /// Поэтому [info] обязан переживать перезапуск целиком: он и переживает —
  /// `toJson`/`fromJson` кладут его вложенным объектом `info`, а `expiresAt`
  /// внутри пишется в UTC (см. [SubscriptionInfo.toJson]).
  ///
  /// Профиль, записанный версией без метаданных, отдаёт
  /// [SubscriptionInfo.empty] → `expiresAt == null` → НЕ помечается. Это и есть
  /// миграция: «дата неизвестна» не равно «подписка истекла», и красить старые
  /// профили в истёкшие мы не имеем права.
  bool isExpiredAt(DateTime now) => info.isExpiredAt(now);

  /// То же на текущий момент.
  bool get isExpired => info.isExpired;
