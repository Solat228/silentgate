...
class MainActivity : FlutterActivity() {
    ...
    /**
     * ⚠️ СТРАХОВКА ОТ «ПОДКЛЮЧЕНИЕ…» НАВСЕГДА.
     *
     * Найдено живой проверкой 16.08.2026 на эмуляторе и записано в BACKLOG #28:
     * если диалог согласия на VPN НЕ ВЕРНЁТ результат, `onActivityResult` не
     * вызовется никогда — и `pendingResult` останется невыполненным. Dart в это
     * время ждёт ответа `invokeMethod('start')`, поэтому приложение зависает в
     * состоянии «Подключение…» насмерть: следующая команда `silentgate://connect`
     * тоже ничего не делает, помогает только перезапуск.
     *
     * Случаев, когда результата не будет, несколько, и все они бытовые: человек
     * свернул приложение поверх диалога, систему заставили пересоздать активность
     * (поворот экрана, нехватка памяти), диалог закрыла сама оболочка.
     *
     * Поэтому при возврате в приложение проверяем ФАКТ, а не ждём события: если
     * ответ ещё висит, а согласие по-прежнему не выдано (`VpnService.prepare`
     * снова вернул интент) — значит его не выдали, и надо честно сказать об этом,
     * а не молчать.
     *
     * ⚠️ Проверка `prepare` обязательна: без неё мы объявляли бы отказом и тот
     * случай, когда согласие УЖЕ выдано, а `onActivityResult` просто ещё не
     * успел отработать — и рвали бы исправное подключение.
     */
    override fun onResume() {
        super.onResume()
        val waiting = pendingResult ?: return
        if (VpnService.prepare(this) == null) return // согласие есть, ждём результат
        pendingConfig = null
        pendingXrayConfig = null
        pendingResult = null
        waiting.error("consent_denied", "Пользователь не разрешил VPN", null)
    }
