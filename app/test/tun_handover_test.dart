...
void main() {
  ...
  final options = ConnectionOptions(
    settings: AppSettings.defaults.copyWith(
      ...
      // ⚠️ Здесь проверяется ПОРЯДОК «подготовка → снятие старого → подъём
      // нового», а он существует только тогда, когда туннель действительно
      // пересоздают. При включённой бесшовности повторный подъём с тем же
      // конфигом туннель не трогает вовсе (`seamless_test.dart`), и порядку
      // просто негде проявиться — тест мерил бы не то, ради чего написан.
