...
void main() {
  ...
  group('Каталог из 14 сервисов', () {
    ...
    test('четырнадцать раскладываются по семь в колонку', () {
      final c = ServiceChecks.columns(ServiceChecks.catalog);
      expect(c.left.length, 7);
      expect(c.right.length, 7,
          reason: 'иначе кнопка Connect перестаёт быть посередине');
    });
