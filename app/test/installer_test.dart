...
void main() {
  ...
  /// ⚠️ СТРОКА ВНУТРИ `[Code]` НЕ ИМЕЕТ ПРАВА НАЧИНАТЬСЯ С `[`.
  ///
  /// ⚠️ ЭТО НЕ ПРИДИРКА К ОФОРМЛЕНИЮ, А ПОЙМАННЫЙ ДЕФЕКТ (10.09.2026).
  /// Inno считает заголовком секции ЛЮБУЮ строку, чей ПЕРВЫЙ НЕПРОБЕЛЬНЫЙ
  /// символ — `[`; отступ от этого не спасает. Многострочный вызов, у которого
  /// массив параметров перенесён на свою строку, — обычное оформление в
  /// Паскале, и глазами оно выглядит безупречно:
  ///
  /// ```
  ...
  /// ```
  ///
  /// ISCC отвечает на это `Error on line NNN: Invalid section tag` и обрывает
  /// компиляцию целиком — установщик не собирается вовсе.
  ///
  /// Почему нужен именно тест, а не внимательность: ISCC на машине разработчика
  /// стоит НЕ в `Program Files` (per-user установка), из-за чего два агента
  /// подряд решили, что компилятора нет, и «проверили» синтаксис чтением. Файл
  /// пролежал сломанным до первой настоящей сборки. Открывающую скобку держать
  /// в КОНЦЕ предыдущей строки.
  test('⚠️ внутри [Code] ни одна строка не начинается с «[»', () {
    final lines = text.split(RegExp(r'\r?\n'));
    final codeAt = lines.indexWhere((l) => l.trimRight() == '[Code]');
    expect(codeAt, greaterThan(0),
        reason: 'секция [Code] не найдена — тест проверял бы пустоту');

    final offenders = <String>[];
    for (var i = codeAt + 1; i < lines.length; i++) {
      final t = lines[i].trimLeft();
      if (!t.startsWith('[')) continue;
      // Настоящий заголовок секции — это `[Имя]` и ничего больше.
      if (RegExp(r'^\[[A-Za-z]+\]\s*$').hasMatch(t)) continue;
      offenders.add('${i + 1}: ${lines[i].trim()}');
    }
    expect(offenders, isEmpty,
        reason: 'ISCC примет это за заголовок секции и откажется собирать '
            'установщик: ${offenders.join(" | ")}');
  });
