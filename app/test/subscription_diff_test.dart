...
void main() {
  ...
  group('Настоящие изменения состава считаются как прежде', () {
    ...
    test('⚠️ ТОТ ЖЕ ХОСТ, ДРУГОЙ ПОРТ, ТО ЖЕ ИМЯ — ТЕПЕРЬ ОБНОВЛЕНИЕ', () {
      // ⚠️ ТЕСТ ПЕРЕВЁРНУТ 02.09.2026 ПО ТРЕБОВАНИЮ ВЛАДЕЛЬЦА: «обновление
      // сервера, а не полная замена, если название совпадает, а различаются
      // только некоторые параметры». Порт — такой же параметр, как отпечаток
      // и sni, которые обновлением считались и раньше.
      //
      // Прежняя редакция ждала здесь «+1 · −1». Цена решения названа честно:
      // два РАЗНЫХ сервера, у которых совпали и протокол, и имя, а разошёлся
      // только порт, сольются в одну запись. У панели имена уникальны, а
      // случай «два узла с одинаковым именем» разводится в `_byIdentity`
      // адресом и портом — но лишь когда оба лежат в ОДНОМ списке.
      final r = diff([parse(link(port: 443))], [parse(link(port: 8443))]);
      expect(r.added, isEmpty);
      expect(r.removed, isEmpty);
      expect(r.keyChanges, hasLength(1),
          reason: 'смена порта обязана быть названа обновлением');
    });

    test('⚠️ два узла с ОДНИМ именем в одном списке не схлопываются', () {
      // Обратная сторона правки выше. Если панель прислала два сервера с
      // одинаковым протоколом и именем, диф обязан видеть оба — иначе он
      // объявил бы один пропавшим на ровном месте.
      final a = parse(link(port: 443, name: 'Дубль'));
      final b = parse(link(port: 8443, name: 'Дубль'));
      final r = diff([a, b], [a, b]);
      expect(r.added, isEmpty);
      expect(r.removed, isEmpty);
      expect(r.total, 2, reason: 'оба сервера на месте, а не один');
    });
  ...
  test('⚠️ СМЕНА АДРЕСА У СЕРВЕРА С ТЕМ ЖЕ ИМЕНЕМ — ОБНОВЛЕНИЕ, а не замена',
      () {
    // Требование владельца 02.09.2026, дословно: «обновление сервера, а не
    // полная замена, если название совпадает, а различаются только некоторые
    // параметры».
    //
    // Как это выглядело у него на экране: панель поправила серверам адрес (в
    // журнале «изменились поля: address, fingerprint»), и пять «Германия 2.x»
    // встали в баннер ОДНОВРЕМЕННО с плюсом и с минусом — «+5 · −6» из одних и
    // тех же имён. Человек читает это как «подписка перетасовала мне половину
    // серверов», хотя не изменилось ничего, кроме адреса.
    //
    // ⚠️ Отпечаток и sni как обновление считались УЖЕ (тесты выше). Адрес — нет:
    // он входил в тождество сервера.
    final before = parse(link(host: 'old.example.org', name: 'Германия 2.2'));
    final after = parse(link(host: 'new.example.org', name: 'Германия 2.2'));

    final r = diff([before], [after]);
    expect(r.added, isEmpty, reason: 'сервер не появлялся — он был');
    expect(r.removed, isEmpty, reason: 'сервер не исчезал — он на месте');
    expect(r.keyChanges, hasLength(1),
        reason: 'это обновление, и оно обязано быть названо обновлением');
    expect(r.keyChanges.single.fields, contains('address'));
  });

  test('⚠️ граница: переименование ОСТАЁТСЯ заменой', () {
    // Без неё правка выше означала бы «склеиваем что попало». Человек узнаёт
    // узел по имени: «Германия 2.2» вместо «Германия 2.3» — другой сервер.
    final before = parse(link(name: 'Германия 2.2'));
    final after = parse(link(name: 'Германия 2.3'));
    final r = diff([before], [after]);
