...
void main() {
  ...
  group('Смена сервера: конфиг TUN', () {
    ...
    test('новое подключение обнуляет накопленные адреса', () async {
      // Правило «мимо туннеля» выпускает трафик к этим адресам МИМО VPN.
      // Сервер прежней подписки, оставшийся в нём навсегда, — это тихая дыра,
      // а не мелкая неточность.
      final s = _settings();
      final e = _FakeEngine()..fallbackServers = [_a];
      await tunCfgOf(e, [_a], s);
      ...
      expect(ips, ['198.51.100.7'],
          reason: 'адрес прежнего подключения (203.0.113.10) обязан уйти');
    });
