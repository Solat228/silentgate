...
void main() {
  ...
  group('⚠️ замер: значок сервиса после сжатия низа', () {
    ...
    testWidgets('⚠️ на 1024×781 после компактной карточки — тоже ШИРИНА',
        (t) async {
      // История этого стража в два шага, и оба шага — замер, а не мнение.
      //
      // Шаг 1 (10.09.2026). Утверждение «31,3 px, упирается ШИРИНА» было снято
      // с ФИКСИРОВАННОЙ коробки 595×438, то есть с высоты, которой на настоящем
      // окне 1024×781 у панели не было. Замер дал 30,96: значок ещё рос от
      // высоты, и 31,3 набиралось только с 1024×900. Тест тогда закрепил
      // именно это: `short < tall`.
      //
      // Шаг 2 (11.09.2026). Карточка подписки на низком окне стала компактной
      // (`SubscriptionBar.compactBelowHeight`), и высвобожденной высоты хватило:
      // на 1024×781 значок вырос до тех же 31,30, что и на 1024×900. Значит
      // теперь И на этом окне ограничивает ширина панели, а не высота, — и
      // страж закрепляет уже этот факт.
      //
      // ⚠️ Почему не «ослабили тест, чтобы прошёл»: прежнее `short < tall`
      // стало ложным по той самой причине, ради которой карточку и сжимали.
      // Если значок на 781 снова станет МЕНЬШЕ, чем на 900, — значит карточка
      // перестала сворачиваться и высота вернулась ограничителем. Это регресс,
      // и он обязан краснеть.
      await pumpHome(t, size: const Size(1024, 781));
      ...
      expect(short, closeTo(tall, 0.5),
          reason: 'на 1024×781 значок снова мельче, чем на 1024×900 — высота '
              'опять ограничивает рост: компактная карточка подписки перестала '
              'сворачиваться на низком окне');
      // Потолок по ширине на панели этого окна — 31,3 px.
      expect(short, closeTo(31.30, 0.15));
    });
    ...
    testWidgets('замер на панелях 535 и 595 из брифа', (t) async {
      ...
      for (final box in const [Size(535, 383), Size(595, 438)]) {
        ...
        // у панели может и не быть. До 11.09.2026 на настоящем окне 1024×781
        // значок был 30,96 — высота ограничивала. После компактной карточки
        // подписки на низком окне высоты хватает, и 31,3 достигается уже на
        // 781 (тест «на 1024×781 после компактной карточки — тоже ШИРИНА»).
        // Совпадение чисел коробки и окна — следствие той правки, а не
        // доказательство, что коробка «правильно» моделирует окно.
