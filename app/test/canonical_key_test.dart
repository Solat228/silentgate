...
void main() {
  ...
  group('Тождество сервера для дифа подписки', () {
    ...
    test('⚠️ разный порт при ТОМ ЖЕ имени — ТОТ ЖЕ сервер (обновление)', () {
      // ⚠️ ПЕРЕВЁРНУТ 02.09.2026 ПО ТРЕБОВАНИЮ ВЛАДЕЛЬЦА: «обновление сервера,
      // а не полная замена, если название совпадает, а различаются только
      // некоторые параметры». Тождество больше не содержит ни адреса, ни порта.
      final a = ShareLinkParser.tryParse(vlessGrpc(serviceParam: 'path'))!;
      final b = ShareLinkParser.tryParse(
  ...
  group('Профиль «Авто …» опознаётся по имени', () {
    ...
    test('⚠️ обычный сервер опознаётся ПО ИМЕНИ, а не по адресу', () {
      // ⚠️ ПЕРЕВЁРНУТ 02.09.2026. Прежде тождество включало адрес, и панель,
      // поправив его, выдавала «+5 · −6» из одних и тех же имён — сервер
      // одновременно «удалён» и «добавлен». Теперь правило одно для всех:
      // совпало имя (и протокол) — тот же сервер, остальное показывается как
      // изменившиеся поля.
      final a = ShareLinkParser.tryParse(vlessGrpc(serviceParam: 'path'))!;
      expect(a.identity, isNot(contains(host)),
          reason: 'адрес в тождестве возвращает исходный дефект');
      expect(a.identity, contains('Москва 1. GRPC'),
          reason: 'опознаём по имени — оно и должно быть в тождестве');
    });
  });
}
