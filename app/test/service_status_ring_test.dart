...
void main() {
  ...
  test('⚠️ подпись почти не шире ячейки — иначе она душит размер значков', () {
    ...
    //
    // ⚠️ Небольшой запас всё же нужен и ИЗМЕРЕН: «Мессенджеры» шрифтом 8
    // занимают около 50 px, и в ячейку 48 не влезают — Flutter рвёт слово по
    // буквам. Потолок здесь стережёт, чтобы запас не начал расти: каждые
    // четыре пикселя подписи стоят двух пикселей размера значка.
    expect(ServiceChecksSides.labelWidthFor(false), lessThanOrEqualTo(54.0),
