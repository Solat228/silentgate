...
> Статус: **v1.4.1 (бета)**. Первый таргет — **Windows**, идёт порт на **Android**. Все режимы
> прогнаны живым тестом в изолированной VM на реальных подписках.
>
> **Что изменилось в 1.4.1:** разбор жалоб. Правило раздельного туннелирования «по имени» теперь
> и правда опознаётся по имени: раньше запись хранилась по полному пути, и обновление программы
> заводило дубль — на одну программу могло накопиться четыре внешне неразличимые строки.
> Плашка пинга больше не объявляет сервер рабочим авансом, пока проверка канала не прошла.
> Экран автонастройки не теряет список серверов при уходе и возврате, показывает таймер и
> отдаёт ход одной кнопкой вместо двух. Логи перестали портиться нулевыми байтами и получили
> срок хранения.
>
