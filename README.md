...
### 1.11.3 — значки проверок вдвое крупнее, состояние — обводкой значка

### 1.11.2 — проверки сервисов влезли в окно: блоки сеткой, а не колонкой

### 1.11.1 — разбор жалоб: восстановление за секунды вместо трёх минут

- **Экран логов читается глазами, а не вычитывается.** Ошибки красным, предупреждения янтарным,
  отладочные приглушённо, время отдельной колонкой; журналы приложения и ядра приведены к одному
  виду времени. Раскраска живёт только в показе — файл, уходящий в поддержку, не меняется ни на
  байт. Заодно экран перестал вставать: раньше он разбирал весь накопленный журнал в каждом кадре
  дважды в секунду, теперь показывает хвост — цена кадра не зависит от размера файла.
- **Журнал ядра больше не показывает адреса ваших серверов.** На этой вкладке маскировки не было
  ни при открытии, ни при копировании, хотя в отчёте поддержки она работала давно.
- **Авария приложения попадает в журнал.** Обработчиков необработанных ошибок не было ни одного:
  Flutter печатал их в консоль, которой у собранного приложения нет, и жалоба «иногда не
  включается» была неразбираема. Теперь причина и первые кадры стека пишутся на диск синхронно —
  падающий процесс до обычной, отложенной записи не доживает.
- **Системный прокси проваливался молча.** Код возврата `reg add` не смотрел никто: при запрете
  записи в реестр приложение показывало «Подключено», а трафик шёл мимо туннеля. Теперь неудача
  видна, а включение и снятие наконец оставляют след — до этой правки во всём журнале не было ни
  одной строки о способе захвата, который на Windows стоит по умолчанию.
- **Переподключение больше не предлагается там, где конфиг не меняется.** Пароль на локальный
  прокси, настройки API и уровень лога ядра требовали его в любом режиме захвата — и, что хуже,
  переподключение читалось как «применилось».
- **Приложение само чинит устаревшую задачу Планировщика**, из-за которой туннель мог перестать
  подниматься после переноса папки.
- **Экран логов переписан**: живое обновление вместо кнопки, выбор что именно чистить, режим
  логирования в настройках — отдельно приложение, отдельно ядро.
- **Пинг при включённом VPN больше не врёт**: было 27 серверов из 52 «быстрее 10 мс».
...
| Ветка | Что в ней | Теги |
|---|---|---|
| `main` | **только стабильные релизы** | `vX.Y.Z` на каждом слиянии |
| `dev` | текущая работа, отсюда собираются беты | `vX.Y.Z-beta.N`, на GitHub — пре-релиз |

Слияние `dev → main` происходит **только в момент стабильного выпуска** — то есть пересекаются
эти две ветки ровно на релизах и больше нигде. Летопись изменений ведут `CHANGELOG.md`
(что и почему поменялось), [docs/ROADMAP.md](docs/ROADMAP.md) (куда идём) и
[docs/BACKLOG.md](docs/BACKLOG.md) (что известно и не сделано).

⚠️ **Бета-сборка помечает себя сама.** Признак канала зашивается во время сборки
(`--dart-define=SILENTGATE_CHANNEL=beta`), и «О программе» пишет «(бета)». Манифестом обновлений
это решить нельзя: канал — свойство самой сборки, а не ответа сервера. Приложение узнаёт о новых
бетах только при включённой галочке «Получать бета-версии» — в основной канал автообновления они
не попадают никогда.
