...
> Статус: **v1.0.3 (бета)**. Первый таргет — **Windows**, идёт порт на **Android**. Все режимы
> прогнаны живым тестом в изолированной VM на реальных подписках.
>
> **Что изменилось в 1.0.3:** починены восемь правил, которые лежали в конфиге и не срабатывали
> никогда. Главное: блокировка приложения пропадала, если у неё стояла галочка «важнее правил
> сайтов»; поддомен проигрывал родителю с другим правилом (сайт, помеченный «через VPN», выходил
> под реальным IP); на Android приложение «прямо» отменяло все правила сайтов, «Авто (лучший
> сервер)» не подключался вообще, а kill switch снимал туннель вместо того, чтобы удержать.
